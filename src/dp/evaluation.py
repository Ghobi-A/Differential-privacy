"""Evaluation utilities for privacy-related checks."""
...
def check_l_diversity(
    df: pd.DataFrame, quasi_columns: list[str], sensitive_column: str, l: int
) -> bool:
...
def check_t_closeness(
    df: pd.DataFrame, quasi_columns: list[str], sensitive_column: str, t: float
) -> bool:
    ...
    for _, group in df.groupby(quasi_columns):
        ...
        aligned = (
            overall_dist.to_frame("overall")
            .join(group_dist.to_frame("group"), how="outer")
            .fillna(0)
        )
